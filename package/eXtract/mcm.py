...
def compute_mcm(hic_matrix: np.ndarray,
    ...
    hic_matrix = (hic_matrix > 0).astype(int)  # Optionally binarize contacts (if needed)
    ...
    def compute_distance_classes(hic_matrix, bin_size=1_000_000,
        ...
        # Generate a distance matrix in Mb
        i_coords, j_coords = np.indices((N, N))
        dist = np.abs(i_coords - j_coords) * (bin_size / 1_000_000.0)  # Distance in Mb

        # Use only the upper triangle above the diagonal to avoid double-counting (and skip diagonal)
        ...
        # Mid contacts are those that are neither near nor far
