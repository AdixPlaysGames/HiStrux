...
def frame_initiation(matrix: np.ndarray, 
    ...
    gaussian_params_weak = dict(epsilon = 10.0, sigma = 0.5)
    ...
    colision_force_weak = gaussian
    colision_force_weak_params = gaussian_params_weak
    ...
    for chs in itertools.combinations_with_replacement(bins.chrom.unique(), 2):
        ...
        colision_force_weak.params[bond_target] = colision_force_weak_params
        
    forces = [chain_force, contact_force, colision_force, colision_force_weak]
...
def run_sim(state_dir: str, 
    ...
    simple : Optional[bool] = False) -> None
        When set to True will perform simulation without forces manipulations and for reduced number of 8e4 steps.
    ...
    #     # stage 2 - adding collision
    #     integrator = hoomd.md.Integrator(dt=0.005,
    #                                     methods=[langevin],
    #                                     forces=[chain_force, contact_force, colision_force_weak])
    #     sim.operations.integrator = integrator
    #     sim.run(8e4)

    #     # stage 3 - full collision
    #     integrator = hoomd.md.Integrator(dt=0.005,
    #                                     methods=[langevin],
    #                                     forces=[chain_force, contact_force, colision_force])
    #     sim.operations.integrator = integrator
